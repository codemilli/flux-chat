...
class Firechat {
    constructor(appId, options) {
        this._firebase = typeof appId !== 'object' ?
            new Firebase(appId || config.firebaseUrl) :
            appId;
    ...
    setUser(userId, userName, callback) {
        this._firebase.onAuth((authData) => {
            if (authData) {
                ...
                this._userName = userName && userName.toString() || 'anonymous';
     ...
     */
    _onRoomAdded(room) {
        this._invokeEventCallbacks('room-added', room);
    }
